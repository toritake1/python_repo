...
CK = 'hHT8XYqZ82XORyB2mJsH8Vea7'                             # Consumer Key
CS = 'fzuUShTzcrgZyub6VDtrDfTkjq37Ebetwd8uRP27QBh47Z24Dz'    # Consumer Secret
AT = '4659503593-LRMk3RNnpaJuDHkhb0pfXRtR1wr1SnYO77nyRqn'    # Access Token
AS = '1lxF7OLslZADMwt4KiGyldewjLujiQPZabDorkpRoM7Fy'         # Accesss Token Secert
...
class TweetsGetterByUser(TweetsGetter):
    ...
    ユーザーを指定してツイートを取得
    '''
    def __init__(self, screen_name, max_id):
...
if __name__ == '__main__':
    ...
    # キーワードで取得
    #getter = TweetsGetter.bySearch(u'クラウド')
    
    # ユーザーを指定して取得 （screen_name）
    getter = TweetsGetter.byUser('idcfrontier','907094510580068352')
 
    cnt = 0
    for tweet in getter.collect(total = 10):
        cnt += 1
        print ('------ %d' % cnt)
        print ('{} {} {}'.format(tweet['id_str'], tweet['created_at'], '@'+tweet['user']['screen_name']))
        print (tweet['text'])
       #
       #print tweet
